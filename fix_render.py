...
replacement = """                const safeName = prod.nombre.replace(/'/g, "\\\\\\'").replace(/"/g, '&quot;');
...
content = re.sub(r'const safeName =.*?</span\s*>', replacement, content, flags=re.DOTALL)

with open('producto.html', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated product rendering.")
