...
"""
# Insert before <!-- CONTENEDOR EXCLUSIVO PARA IMPRESIÓN -->
...
js_functions = """
        ...
        window.calcularReventa = function() {
            ...
            // Obtener tasa BCV global del input oculto/visible de facturación o el valor por defecto si no ha cargado
            const inputTasa = document.getElementById('fac-tasa');
            const tasaVES = inputTasa && inputTasa.value ? parseFloat(inputTasa.value) : 1;
            
            // Asumiendo paridad 1.1 para EUR si no hay tasa EUR definida explícitamente en el DOM, o usar otra variable si existe
            let tasaEUR = 1.05; // Default fallback if no EUR rate available directly, but let's try to get it
        ...
        window.cargarHistorialReventas = async function() {
            ...
            try {
                ...
                reventas.forEach(rev => {
                    ...
                    const ventaVES = ((rev.ventaUSD || 0) * (rev.tasaCambioVES || 1)).toFixed(2);
                    const netaVES = ((rev.gananciaNetaUSD || 0) * (rev.tasaCambioVES || 1)).toFixed(2);
        ...
        window.eliminarReventa = async function(id) {
            if (!confirm("¿Estás seguro de eliminar esta operación de reventa?")) return;
            try {
                await deleteDoc(doc(db, "reventas_gastos", id));
                window.cargarHistorialReventas();
            } catch (error) {
